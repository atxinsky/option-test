...
class BullCallSpread(BaseOptionStrategy):
    """牛市Call价差策略"""

    name = "bull_call_spread"
    display_name = "牛市Call价差"
    description = "趋势看涨时，买入ATM Call + 卖出OTM Call，风险有限盈利有限"
    version = "1.0"
    ...
    def get_params(cls) -> List[StrategyParam]:
        return [
            ...
            StrategyParam("iv_percentile_max", "IV百分位上限", 50, 20, 80, 5, "int",
                         description="IV百分位超过此值不入场"),
            StrategyParam("delta_buy", "买入腿Delta", 0.50, 0.40, 0.60, 0.05, "float",
                         description="买入Call的目标Delta"),
            StrategyParam("delta_sell", "卖出腿Delta", 0.30, 0.20, 0.40, 0.05, "float",
                         description="卖出Call的目标Delta"),
            StrategyParam("spread_width", "价差宽度", 100, 50, 200, 25, "int",
                         description="两腿行权价间距（点）"),
    ...
    def _check_entry(self, bar, spot, option_chain, iv_percentile,
        ...
        # 获取价格
        buy_price = self.get_option_price(buy_strike, 'call', option_chain,
                                          spot, T, current_iv)
        sell_price = self.get_option_price(sell_strike, 'call', option_chain,
                                           spot, T, current_iv)
        ...
        return Signal(
            action='open',
            legs=legs,
            tag='bull_call_spread',
            reason=f"EMA金叉，趋势看涨，IV={iv_percentile:.0f}%",
    ...
    def _check_exit(self, bar, spot, option_chain, iv_data,
        ...
        # 计算当前损益和指标
        entry_cost = self.position.get_entry_cost()  # 负值表示支出
        current_value = sum(leg.current_price * leg.position for leg in self.position.legs)
        pnl = current_value - entry_cost

        spread_width = abs(self.position.legs[1].strike - self.position.legs[0].strike)
        max_profit = spread_width + entry_cost  # entry_cost是负值
        ...
        # 条件3: 时间价值损耗过多且方向不利
        if entry_cost < 0:  # 支出权利金
            loss_ratio = pnl / entry_cost  # 亏损比例
